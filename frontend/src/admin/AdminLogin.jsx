...
import { Shield, Lock, ArrowRight, AlertCircle, Eye, EyeOff, Check, X } from "lucide-react";
...
const AdminLogin = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-12 relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 left-0 right-0 h-[300px] bg-gradient-to-b from-[#02225b]/10 to-transparent -z-10" />

      <div className="w-full max-w-md">
        <div className="bg-white rounded-[32px] p-8 shadow-2xl shadow-slate-200/50 border border-slate-100 sm:p-10">
          {/* Header */}
          <div className="mb-10 text-center">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-[#02225b] to-indigo-600 mb-4">
              <Shield className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-4xl font-black text-[#02225b] tracking-tight">Admin Access</h1>
            <p className="mt-3 text-slate-500 font-medium">Restricted area • Authorized personnel only</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              <label htmlFor="email" className="text-sm font-bold text-slate-600 flex items-center gap-2 px-1">
                <Shield className="w-4 h-4 text-[#02225b]" /> Admin Email
              </label>
              <div className="relative group">
                <input
                  ...
                  id="email"
                  name="email"
                  placeholder="admin@casecraft.com"
                  value={form.email}
                  ...
                  className={`w-full rounded-2xl border-2 bg-slate-50/50 px-5 py-4 text-slate-700 transition-all focus:bg-white outline-none ${validationErrors.email && touched.email
                    ...
                    : "border-slate-100 focus:border-[#02225b]"
                    }`}
                  required
                />
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between px-1">
                <label htmlFor="password" className="text-sm font-bold text-slate-600 flex items-center gap-2">
                  <Lock className="w-4 h-4 text-[#02225b]" /> Password
                </label>
              </div>
              <div className="relative group">
                <input
                ...
                <button
                  ...
                  className="absolute inset-y-0 right-0 flex items-center pr-4 text-slate-400 hover:text-[#02225b] transition-colors"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#02225b] to-indigo-600 px-6 py-4 text-white font-black text-lg shadow-xl shadow-[#02225b]/20 hover:shadow-2xl hover:shadow-[#02225b]/30 active:scale-95 disabled:opacity-60 transition-all"
            >
              {loading ? (
                <>
          ...
          <div className="mt-10 space-y-4">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-slate-400 font-bold tracking-widest">info</span></div>
            </div>

            <div className="text-center space-y-3">
              <p className="text-sm font-medium text-slate-500">
                Not an admin?{" "}
                <Link to="/" className="font-bold text-[#02225b] hover:text-indigo-600 transition-colors">
                  Return to Home
                </Link>
              </p>
